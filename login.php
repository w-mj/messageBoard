...
elseif (isset($_COOKIE['saveuser']) && $_COOKIE['saveuser'] != '') {
    ...
    $result = $db -> query($query) or die('数据库查询失败1'.mysqli_error($db));
...
if (isset($_POST['username']) && $_POST['username'] != ''
    ...
    ($db = New mysqli('localhost', 'msgboard', 'root', 'msgBoard')) or die('数据库连接失败2');
    ...
    $result = $db->query($query) or die('数据库查询失败3'.mysqli_error($db));
    ...
    if (isset($_SESSION['loginTimes']) && $_SESSION['loginTimes'] != 0)
